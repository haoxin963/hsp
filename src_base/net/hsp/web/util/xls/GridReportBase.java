...
public class GridReportBase {
	public SimpleDateFormat timeFormat = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");

	private static String MODULE_PATH = "../../view/sys/export/module.xls";//模板路径
	...
	public GridReportBase() {
	}

	public GridReportBase(HttpServletRequest request,
	 ...
	 * @throws Exception
	 */
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData,boolean showTitle,Map headerMap) throws Exception {
		...
		if(showTitle){
			...
			if(headerMap != null){
				Object[] keyArray = headerMap.keySet().toArray();	
				for(int i = 0; i < keyArray.length; i++){
					Object value = headerMap.get(keyArray[i]);
		...
		/* 设置默认宽度 */  
		String widths = request.getParameter("defaultColWidth");
		String[] defaultColWidths = {};  
		if(StringUtils.isNotBlank(widths)) {
			defaultColWidths = widths.split(",");
		}  
		if( defaultColWidths.length > 0) {
			int colsize = headerMetaData.getColumns().size();
			for (int i = 0; i < defaultColWidths.length; i++) {  
				if(i > colsize-1) break;
				int curColWidth = sheet.getColumnWidth(i);
			    int defaultColWidth = 1;
		        try {  
		        	defaultColWidth = Integer.parseInt(defaultColWidths[i]);  
		        } catch (Exception e) {  
		             continue;
		        }		    
			    if (curColWidth < defaultColWidth || defaultColWidth == 0) {  
	 ...
	 */
	public void exportToExcel(String title, String sheetName,String creator, TableData tableData,boolean showTitle,Map headerMap)
			throws Exception {

		HSSFWorkbook wb = new HSSFWorkbook();// 创建新的Excel 工作簿
		HashMap<String, HSSFCellStyle> styles = initStyles(wb);// 初始化表头样式
		
		wb = writeSheet(wb,sheetName,styles,creator,tableData,showTitle,headerMap);//写入工作表
		title += ".xls";
		
		response.setHeader("Content-Disposition", "attachment;filename=".concat(title));
	...
	public void exportToExcel(String title, String sheetName,String creator, TableData tableData,boolean showTitle)
			throws Exception {

		HSSFWorkbook wb = new HSSFWorkbook();// 创建新的Excel 工作簿
		HashMap<String, HSSFCellStyle> styles = initStyles(wb);// 初始化表头样式
		
		wb = writeSheet(wb,sheetName,styles,creator,tableData,showTitle,null);//写入工作表
		title += ".xls";
	 ...
	 *         bgcolor:#CAEAFE; </style>
	 */
	public void exportToExcel(String title, String creator, List<TableData> tableDataLst,boolean showTitle)
		throws Exception {
		...
		for(TableData tableData : tableDataLst){
			...
			wb = writeSheet(wb,tableData.getSheetTitle(),styles,creator,tableData, showTitle , new HashMap());//写入工作表
			i++;
		}
