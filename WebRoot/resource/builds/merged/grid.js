    ...
    $.fn.bsgrid = {
        ...
        init: function (gridId, settings) {
            ...
            if (!options.settings.pageAll) {
                ...
                try {
                    ...
                    //$('#' + options.pagingOutTabId).width($('#' + gridId).width());//宽度计算bug
        ...
        parseJsonData: {
            ...
            totalRows: function (json) {
            	if (json.command.total == undefined) {
            		json.command.total = json.command.rows.length;//haoxin
            	}
                return json.command.total;
            },
            curPage: function (json) {
            	if (json.command.page == undefined) {
            		json.command.page = 1;//haoxin
            	}
                return json.command.page;
            },
            data: function (json) {
                return json.command.rows;
            },
...
(function ($) {
        ...
        $.fn.bsgrid.getGridHeaderObject(options).each(function (hi) {
            if (options.columnsModel[hi].check == 'true') {
                ...
                $(this).find('input[type=checkbox]').change(function () {
                    ...
                    $.fn.bsgrid.getRows(options).find('td:nth-child(' + (hi + 1) + ')>input[type=checkbox]').change();
                });
            }
        });
        ...
        $.fn.bsgrid.getGridHeaderObject(options).each(function (hi) {
            if (options.columnsModel[hi].check == 'true') {
                ...
                checkboxObjs.change(function () {
                    ...
                    checkboxEvent(this.value,this.checked);
    ...
    //function checkboxEvent(id, type) {};
