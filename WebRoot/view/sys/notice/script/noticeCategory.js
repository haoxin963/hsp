var basePath = path+'/sys/notice/noticeCategory';
var pk = "id";
...
function doLoad(action, id) {
	...
	if (action == "edit") {
		...
		if(nodes.length==0){
			alertError("请在左侧选择所属栏目!");
			return;
		}
		url+="?pId="+nodes[0].id+"&pName="+nodes[0].name;
...
function rowParent(row, rowIndex, colIndex, options)
{
	var str=getRowVal(row, "pName");
	if(str==null||str==""){
		str="全部";
	}
	     return str;
